...
export interface ArmExtractResult {
  architecture: {
    ...
    services: { id: string; name: string; type: string; category: string; description: string; groupId: string | null; resourceName?: string; location?: string }[];
...
interface FlatResource {
  ...
  location: string;     // resolved region, '' when only an unresolvable expression
  dependsOn: string[];
...
/**
 * Resolve an ARM `location` value. Only literals and `parameters('x')` with a
 * defaultValue are resolvable; runtime expressions like `resourceGroup()`
 * return '' rather than a guess.
 */
function resolveLocation(raw: string, paramMap: ParamMap): string {
  const s = String(raw ?? '').trim();
  if (!s) return '';
  if (!s.startsWith('[')) return s;
  const param = s.match(/parameters\(\s*'([^']+)'\s*\)/i)?.[1];
  return param && paramMap[param] ? paramMap[param] : '';
}
...
function flattenResources(resources: any[], paramMap: ParamMap, parentName = ''): FlatResource[] {
  ...
  for (const r of resources) {
    ...
    out.push({ armType, rawType, name: cleanName(rawName, paramMap) || parentName || 'resource', rawName, kind, location: resolveLocation(r.location, paramMap), dependsOn, propsText });
...
export function extractArchitectureFromArm(template: any): ArmExtractResult {
  ...
  for (const fr of flat) {
    ...
    const svc = {
      ...
      location: fr.location || undefined,
    };
    services.push(svc);
